...
function date_en_to_he(cls_id){
    var dateElements = document.querySelectorAll(cls_id);

    if (dateElements.length === 0) {
    console.log("No elements found with the class",cls_id);
    return; // Return early if no elements are found
}
    
    dateElements.forEach(function(dateElement) {
        var dateString = dateElement.textContent.trim(); // Get the date string and remove any leading/trailing spaces
        var parsedDate = new Date(dateString);
        
        // Convert the date to Hebrew using Intl.DateTimeFormat
        var hebrewDate = new Intl.DateTimeFormat('en-u-ca-hebrew', { dateStyle: 'full', timeZone: 'UTC' }).format(parsedDate);
        
        // Update the content of the element with the Hebrew date
        dateElement.textContent = hebrewDate;
    });
}

window.onload = function() {
    date_en_to_he(".post-meta time.dt-published");
    date_en_to_he(".post-meta");
};
